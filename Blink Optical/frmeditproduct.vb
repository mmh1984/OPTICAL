...
Public Class frmeditproduct
    ...
    Private Sub update_product()
        Try
            open_connection()
            Dim sql As String
            sql = "UPDATE tblproducts SET producttype=@v1,productcode=@v2,productname=@v3,productbrand=@v4,productmodel=@v5,"
            ...
            If (cmd.ExecuteNonQuery) Then
                ...
                cmd.Dispose()
                conn.Close()
                frmproducts.tabproducts.SelectedIndex = 0
    ...
    Private Sub delete_product()
        Try
            open_connection()
            Dim sql As String = "UPDATE tblproducts SET productstatus='deleted' WHERE productid=@id"
            cmd = New SqlCommand(sql, conn)
            cmd.Parameters.AddWithValue("@id", pid)
            If (cmd.ExecuteNonQuery) Then
   ...
   e.KeyChar <> ControlChars.Back AndAlso e.KeyChar <> "." Then 'allows use of backspace
    ...
    Private Sub save_product()
        Try
            ...
            If (cmd.ExecuteNonQuery) Then
                MessageBox.Show("Products Added!", "Products", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                Me.Close()
            End If
        Catch ex As Exception
